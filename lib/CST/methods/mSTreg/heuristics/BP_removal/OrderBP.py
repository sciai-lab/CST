...
class OrderBP4removal():
    ...
    def put(self,return_dict_changed=False, **kwargs):
        if return_dict_changed:
            return {}
...
class OrderBP4removal_closest(OrderBP4removal):
    ...
    def put(self,coords,T_dict,merging_node,return_dict_changed=False,**kwargs):
        if return_dict_changed:
            dict_changed={}
        neighbors_idx_closest_neighbor = T_dict[merging_node]+[merging_node]
        for node in neighbors_idx_closest_neighbor:
            if node>=self.n:
                neighbors_node = T_dict[node]

                D2node = np.linalg.norm(coords[node] - coords[neighbors_node], axis=1)

                idx = np.argmin(D2node)
                if return_dict_changed:
                    dict_changed[node] = self.Q[node]
                self.Q[node] = (D2node[idx],len(neighbors_node))
                
                minimizer_neighbor=neighbors_node[idx]
                if minimizer_neighbor>=self.n and len(T_dict[minimizer_neighbor])<len(T_dict[node]):
                    self.Q[minimizer_neighbor] =min(self.Q[minimizer_neighbor], (D2node[idx], len(T_dict[minimizer_neighbor])))
                # else:
                #     self.Q[node] = (D2node[idx], len(neighbors_node))
        if return_dict_changed:
            return dict_changed
    ...
    def put(self, coords, T_dict, merging_node,return_dict_changed=False, **kwargs):
        ...
        for node in neighbors_idx_closest_neighbor:
            if node >= self.n:
                ...
                idx = np.argmax(D2node)
                if return_dict_changed:
                    dict_changed[node] = self.Q[node]
                self.Q[node]=(-D2node[idx],len(neighbors_node))
                
                maximizer_neighbor=neighbors_node[idx]
                if maximizer_neighbor>=self.n:
                    self.Q[maximizer_neighbor] =min(self.Q[maximizer_neighbor], (-D2node[idx], len(T_dict[maximizer_neighbor])))
                ...
                for neighbor in T_dict[neighbor_merging_node]:
                    if neighbor < self.n:
                        if updated:
                            ...
                            self.Q[neighbor_merging_node]=min(self.Q[neighbor_merging_node],(distance,num_neighbors))
                        else:
                            if return_dict_changed:
                                dict_changed[neighbor_merging_node] = self.Q[neighbor_merging_node]
                            distance=np.linalg.norm(coords[neighbor_merging_node] - coords[neighbor])
                            num_neighbors=len(T_dict[neighbor_merging_node])
